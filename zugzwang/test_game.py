...
class TestZugGameNodeWrapper:
    ...
    def test_bind(self, MyZugGameNodeWrapper, MyZugData):
        """The node's comment is populated with json generated by the data class."""

        # Setup
        node = chess.pgn.Game()
        zug_data = MyZugData(floater=100.0, string='barfoo')
        json_string = ZugJsonTools.encode(zug_data.as_dict())
        expected_comment = ZugStringTools.to_square_braces(json_string)

        # Create the wrapper and edit the data
        wrapper = MyZugGameNodeWrapper(node)
        wrapper.data.floater = 100.0
        wrapper.data.string = 'barfoo'

        # Bind data to game node
        wrapper.bind()

        # Check that the comment reads correctly
        assert node.comment == expected_comment
